...
function Show-Usage {
    ...
    Show-Msg "" -Type 'Information'
    Show-Msg "  vl 34429 `"[AT][前端] 標題A`" `"[AT][前端] 標題B`"" -Type 'Information'
    Show-Msg "    一次建立多張 Task（標題直接接在 ID 後、各自加引號、空白分隔即可），可加選既有 Task，一張 PR 連結全部" -Type 'Information'
    Show-Msg "    也可用具名 + 逗號：vl 34429 -NewTask `"A`",`"B`"" -Type 'Information'
    Show-Msg "" -Type 'Information'
    ...
    Show-Msg "  <標題...>       直接把 Task 標題接在 ID 後（空白分隔、各自加引號）可一次建多張；等同 -NewTask" -Type 'Information'
